...
from intelligentQT import Ui_Main
...
class MainWindow(QMainWindow, Ui_Main):
    ...
    def starton(self):
        if self.count == 0:
            self.th.stop()
            self.count = 1
            self.StartButton.setIcon(QIcon(QPixmap(':/icons/res/drawable/icons/stop.png')))
        else:
            self.th.start()
            self.count = 0
            self.StartButton.setIcon(QIcon(QPixmap(':/icons/res/drawable/icons/pause.png')))
    def Bt_lich(self):
